...
StereoUndistortRos::StereoUndistortRos(
    const YAML::Node& yaml_node, const bool is_right_flipped)
    : is_right_flipped_(is_right_flipped),
      camera_pair_(yaml_node, is_right_flipped) {
  ...
  if (is_right_flipped) {
    pub_image_right_flipped_ = node_handle_.advertise<sensor_msgs::Image>(
        cam_right_adapted_topic_name, kQueueSize);
  }
...
void StereoUndistortRos::imageCallback(
  ...
  if (is_right_flipped_) {
    ...
    pub_image_right_flipped_.publish(cv_img_right->toImageMsg());
    camera_pair_.getCamRightFlipped().undistort_rectify(cv_img_right->image);
  } else {
  ...
  auto cam_right_info =
      is_right_flipped_ ? camera_pair_.getCamRightFlipped().generateCameraInfo()
                        : camera_pair_.getCamRight().generateCameraInfo();
